...
let create (i : Signal.t I.t) =
	...
	let gated_read_reg = mux2 is_read_op memory_outputs.(2) (zero 16) in
...
let%expect_test "REG Test" =
	...
	let run opcode addr_1 addr_2 addr_3 data =
	...
	run 0x1000 1 2 3 25; (* 10 + 15 = 25 *)
	run 0x2200 0 0 3 0;  (* Read REG:3 to see if the result (25) has been stored *)
	
	[%expect {|
   opcode:2100 addr_1:0 addr_2:0 addr_3:1 | REG:1:0 REG:2:0 R_Reg:0
   opcode:2200 addr_1:0 addr_2:0 addr_3:1 | REG:1:0 REG:2:0 R_Reg:15
   opcode:2100 addr_1:0 addr_2:0 addr_3:2 | REG:1:0 REG:2:0 R_Reg:0
