...
let day_1_part_2 = [|
    ...
    (* Start of underflow loop *)
    { addr = 0x0021; data = 0x100f_0400 }; (* 21: ADD REG:F = REG:0 + REG:4 (TEMP = dial_val + 0) To update the flags to check if its negative, dummy result stored in REG:f (TEMP) *)
    { addr = 0x0022; data = 0xf700_0026 }; (* 22: JMPNN 25                 JMPNN Jump if not negative (positive), check if we need to run the underflow loop, if not then this will be true and we will jump over it *)
    { addr = 0x0023; data = 0x1000_0200 }; (* 23: ADD REG:0 = REG:0 + REG:2 (dial_val = dial_val + 100) *)
    { addr = 0x0024; data = 0x1009_0901 }; (* 24: ADD REG:9 = REG:9 + REG:1 (result_val = result_val + 1) Increment the output variable when the result is zero *)
    { addr = 0x0025; data = 0xf000_0021 }; (* 25: JMP 21                   If the exit condition is not met, we loop again, jump to start of loop *)
    ...
    (* end_while *)
    (* Start of overflow loop *)
    { addr = 0x0026; data = 0x110f_0003 }; (* 26: SUB REG:F = REG:3 - REG:0 (TEMP = 99 - dial_val) To update the flags to check if its negative, dummy result stored in REG:f (TEMP) *)
    { addr = 0x0027; data = 0xf700_002b }; (* 27: JMPNN 29                 JMPNN Jump if not negative (positive), check if we need to run the underflow loop, if not then this will be true and we will jump over it *)
    { addr = 0x0028; data = 0x1100_0200 }; (* 28: ADD REG:0 = REG:0 - REG:2 (dial_val = dial_val - 100) *)
    { addr = 0x0029; data = 0x1009_0901 }; (* 29: ADD REG:9 = REG:9 + REG:1 (result_val = result_val + 1) Increment the output variable when the result is zero *)
    { addr = 0x002a; data = 0xf000_0026 }; (* 2a: JMP 25                   If the exit condition is not met, we loop again, jump to start of loop *)
    (* End of overflow loop *)
    
    (* Check loop condition and exit *)
    { addr = 0x002b; data = 0x100f_0804 }; (* 2b: ADD REG:F = REG:8 + REG:4 (TEMP = loop_index + 0) To update flags to check for zero result to exit the loop *)
    { addr = 0x002c; data = 0xf100_002e }; (* 2c: JMPZ 2f                  Jump to end of program if the loop_index equals zero to display thr result and exit the program *)
    { addr = 0x002d; data = 0xf000_001a }; (* 2d: JMP 1a                   Jump to the start of the loop *)
    
    (* Main Loop End *)
    { addr = 0x002e; data = 0x2209_0000 }; (* 2e: Output the result value *)
    { addr = 0x002f; data = 0x0000_0000 }; (* 2f: HALT END PROGRAM *)
